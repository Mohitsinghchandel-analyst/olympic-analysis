...
def medal_tally(df):
    ...
    medal_tally=df.groupby('region')[['Gold', 'Silver', 'Bronze']].sum().sort_values(by=['Gold', 'Silver', 'Bronze'], ascending=False).reset_index()
...
def fetch_medal_tally(df,years,country):
    ...
    if flag==1:
        ...
        x=temp_df.groupby('region').sum()[['Gold', 'Silver', 'Bronze']].sort_values('Gold', ascending=False).reset_index() # Sort by 'Gold' column instead of the non-existent 'Years' column
...
def data_over_time(df,col):
    ...
    nations_over_time = nations_over_time.rename(columns={'Year': 'Edition', col: col})
...
def men_vs_women(df):
    ...
    fem=athlete_df[athlete_df['Sex']=='F'].groupby('Year').count()['Name'].reset_index()  
    final=men.merge(fem,on='Year',how='left')
